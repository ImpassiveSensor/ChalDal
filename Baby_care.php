...
<body>
	...
	if ($sort==1) {
		$query = "select * from item_information where catagory=7 order by Price ASC" ;
	} else if ($sort==2) {
		$query = "select * from item_information where catagory=7 order by Price DESC" ;
	} else if($sort==0) {
		$query = "select * from item_information where catagory=7 order by Id ASC" ;
	}
			...
			while ($item =mysqli_fetch_array($command)) {
				...
				<div class="col-lg-3">
					<form method="post" >
						...
						<div class="card-body">
							<img src="images/<?php echo $item['Image']  ?>" class="img-fluid">
